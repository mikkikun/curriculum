...
-- [回答]
SELECT quantity,goods_name,store_name FROM stock_table JOIN goods_table ON stock_table.goods_code = goods_table.goods_code JOIN store_table ON stock_table.store_code = store_table.store_code;
...
-- [回答]
SELECT store_code, quantity FROM stock_table WHERE quantity > (SELECT AVG(quantity) FROM stock_table WHERE store_code ='EA01’);
